...
const RatingStars = ({ lawyerId }) => {
  ...
  const submitRating = async (rate) => {
    try {
      ...
      await axios.post(
        "https://advocate-connect-backend.onrender.com/rate-lawyer",
...
const Services = () => {
  ...
  const booklawyerfunciton = async (lawyer_id) => {
    ...
    try {
      const response = await axios.post(
        `https://advocate-connect-backend.onrender.com/book-lawyer/${loggedInUser._id}`,
        { lawyerId: lawyer_id }
      );
  ...
  const filtered = lawyers.filter((lawyer) => {
    ...
    const caseMatch = filters.specializedIn
      ? lawyer.specializedIn?.includes(filters.specializedIn)
      : true;
